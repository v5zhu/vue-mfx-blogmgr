<template>
    <div class="animated fadeIn" style="margin-left: 20px;">
        ...
        <Modal v-model="funcModal" width="600" :maskClosable="false"
               @on-visible-change="changeModalVisible">
            <p slot="header" style="color:#f60;text-align:left">
                <Icon type="ios-location-outline" size="20"></Icon>
                <span style="font-size:14px;">新增页面功能</span>
            </p>
            <Form ref="funcForm" :model="func" :label-width="80">
                <Row>
                    <Col span="20" class="link-piece">
                    <ul>
                        <li style="margin: 10px;">
                            <Form-item prop="upper" label="所属路由">
                                <Cascader v-model="func.upper" :data="routesTree" change-on-select
                                          @on-change="changeParent"></Cascader>
                            </Form-item>
                        </li>
                        <li style="margin: 10px;">
                            <Form-item prop="name" label="名称">
                                <Input v-model="func.name" type="text">
                                </Input>
                            </Form-item>
                        </li>
                        ...
                        <li style="margin: 10px;">
                            <Form-item prop="description" label="描述">
                                <Input v-model="func.description" type="text">
                                </Input>
                            </Form-item>
                        </li>
                        <li>
                            <div style="text-align: right;margin: 10px;">
                                <Button type="ghost" @click="clearAll('funcForm')">
                                    <Icon type="ios-checkmark" size="14"></Icon>
                                    清除
                                </Button>
                                <Button type="ghost" @click="saveFunction">
                                    <Icon type="ios-checkmark" size="14"></Icon>
                                    保存
                                </Button>
                            </div>
        ...
        methods: {
            ...
            changeParent(routeId) {
                if (routeId && routeId.length > 0) {
                    this.func.route.id = routeId[routeId.length - 1];
                } else {
            ...
            editFunction(row) {
                ...
                this.func = row;
                this.func.upper = routeTree;//设置上级路由的默认值
            ...
            saveFunction() {
                ...
                if (!this.func.id) {
                    store.dispatch('AddFunction', this.func).then(res => {
                        var data = res.data;
                        if (data.success == true) {
                    ...
                    store.dispatch('EditFunction', this.func).then(res => {
                        var data = res.data;
                        if (data.success == true) {
            ...
            remove(id) {
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        setTimeout(() => {
                            store.dispatch('DeleteFunction', {id: id}).then(res => { // 拉取user_info
                                var resp = res.data;
